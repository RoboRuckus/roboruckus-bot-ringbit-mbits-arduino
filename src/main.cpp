...
/// @brief Pin with button to switch to calibrate mode (hold for ~5 seconds on boot to enter calibrate mode). Button B
...
void setup()
{
    ...
    // Enter calibration mode if necessary
    ...
    // Skip this if in calibration mode
    if (!robot.inCalibrateMode)
    {
        // Configure WiFi
        DNSServer dns;
...
void loop()
{
    ...
    // Check if bot is in calibrate mode and process accordingly
    if (!robot.inCalibrateMode)
    {
    ...
    {
        // Check if the button was pressed to start the turn calibration process
        if (digitalRead(MODE_PIN) == LOW)
        {
            robot.turnCalibration();
            config.saveSettings();
        }
        // Check if the button was pressed to do a linear movement calibration
        else if (digitalRead(RESET_PIN) == LOW)
        {
            // Check if the forward calibration was completed already.
            if (!forwardCalComplete)
            {
                if (robot.linearCalibrationForward())
                {
                    config.saveSettings();
                    forwardCalComplete = true;
                }
            }
            else
            {
                if (robot.linearCalibrationBackward())
                {
                    config.saveSettings();
                    forwardCalComplete = false;
                }
            }
        }
    }
